...
export default function App() {
  ...
  const handleConnect = async (e) => {
    ...
    if (authMethod === 'token') {
      ...
      // AuthenticateByName
      if (!cleanUrl || !formUsername.trim()) {
        setErrorMsg('Please enter both Server URL and Username.');
        return;
      }
      try {
  ...
  useEffect(() => {
    ...
    const fetchPlaybackInfo = async () => {
      try {
        const res = await fetch(`${serverUrl}/Users/${userId}/Items/${activePlayback.id}?Fields=Chapters,MediaSources`, {
          headers: getHeaders()
        });
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    // Polyfill HTML5 video capabilities for Shaka Player
    shaka.polyfill.installAll();

    if (shaka.Player.isBrowserSupported()) {
      playerInstance = new shaka.Player(video);
      hlsRef.current = playerInstance;

      playerInstance.configure({
        streaming: {
          bufferingGoal: 60,            // aggressively cache up to 60 seconds ahead
          rebufferingGoal: 10,          // play when 10 seconds is buffered
          bufferBehind: 30,             // keep 30 seconds back buffer
          lowLatencyMode: true
        }
      });
      ...
      playerInstance.addEventListener('variantchanged', () => {
        ...
        if (activeTrack) {
          setActiveHlsAudioTrack(activeTrack.id);
        }
      });

      playerInstance.load(streamUrl, prevTime > 0 ? prevTime : null)
        .then(() => {
          console.log('Shaka Player: The video has been loaded successfully!');
          const variantTracks = playerInstance.getVariantTracks();
          
          // Filter to unique language/label combinations to show in the UI
          const uniqueAudioTracks = [];
          const seenAudio = new Set();
          for (const track of variantTracks) {
            const key = `${track.language}-${track.label || track.audioCodec || ''}`;
            if (!seenAudio.has(key)) {
              seenAudio.add(key);
              uniqueAudioTracks.push(track);
            }
          }
          setHlsAudioTracks(uniqueAudioTracks);
    ...
    return () => {
      video.removeEventListener('loadedmetadata', restoreTime);
      if (playerInstance) {
        playerInstance.destroy().then(() => {
          if (hlsRef.current === playerInstance) {
            hlsRef.current = null;
          }
        });
      }
  ...
  // Smart Timeline Controls & Chapters helpers
  const currentChapter = chapters.find(ch => {
    const startSecs = ch.StartPositionTicks / 10000000;
    // Find next chapter start position, or default to video duration
    const nextCh = chapters.find(n => n.StartPositionTicks > ch.StartPositionTicks);
    const endSecs = nextCh ? (nextCh.StartPositionTicks / 10000000) : duration;
    return currentTime >= startSecs && currentTime < endSecs;
  });

  const isIntroActive = currentChapter && currentChapter.Name?.toLowerCase().includes('intro');

  const skipIntro = () => {
    if (!videoRef.current || !currentChapter) return;
    const nextCh = chapters.find(n => n.StartPositionTicks > currentChapter.StartPositionTicks);
    const skipTo = nextCh ? (nextCh.StartPositionTicks / 10000000) : duration;
  ...
  const isCreditsActive = currentChapter && (currentChapter.Name?.toLowerCase().includes('credits') || currentChapter.Name?.toLowerCase().includes('outro'));
  const isNearEnd = duration > 0 && currentTime >= duration - 30;
  const isCreditsPeriod = isCreditsActive || isNearEnd;
  const hasNextEpisode = activePlayback && activePlayback.isEpisode && activePlayback.currentEpisodeIndex < activePlayback.episodesList.length - 1;

  useEffect(() => {
    if (isCreditsPeriod && hasNextEpisode) {
      setShowNextEpisodeCard(true);
    } else {
      setShowNextEpisodeCard(false);
      setCountdown(10);
    }
  }, [isCreditsPeriod, hasNextEpisode]);

  useEffect(() => {
    if (!showNextEpisodeCard || !isPlaying) return;
    if (countdown <= 0) {
      handleNextEpisode();
      return;
    }
  ...
  }, [showNextEpisodeCard, countdown, isPlaying]);
      ...
      {!isConnected && (
        <div className="setup-overlay">
          <div className="setup-card">
            ...
            <div className="auth-tabs" style={{ display: 'flex', gap: '10px', marginBottom: '20px', borderBottom: '1px solid #2f275c', paddingBottom: '10px' }}>
              <button
                type="button"
                style={{ flex: 1, padding: '8px', fontSize: '0.85rem', background: authMethod === 'login' ? 'var(--accent-gradient)' : 'transparent', border: authMethod === 'login' ? 'none' : '1px solid #2f275c', color: '#fff', cursor: 'pointer', borderRadius: '4px', fontWeight: 600 }}
              ...
              <button
                type="button"
                style={{ flex: 1, padding: '8px', fontSize: '0.85rem', background: authMethod === 'token' ? 'var(--accent-gradient)' : 'transparent', border: authMethod === 'token' ? 'none' : '1px solid #2f275c', color: '#fff', cursor: 'pointer', borderRadius: '4px', fontWeight: 600 }}
                onClick={() => { setAuthMethod('token'); setErrorMsg(''); }}
              >
                API Token
              </button>
            </div>
        ...
        <>
          ...
          {activePlayback && (
            ...
            >
              ...
              {/* Skip Intro Overlay */}
              {(isIntroActive || (!currentChapter && activePlayback.isEpisode && duration >= 180 && currentTime >= 10 && currentTime <= 90)) && (
                <button 
                  className="btn btn-secondary"
                  onClick={(e) => {
                    e.stopPropagation();
                            ...
                            <div className="tracks-column">
                              ...
                              {audioStreams.length > 0 ? (
                                audioStreams.map(stream => {
                                  const isActive = selectedAudioIndex === stream.Index || (selectedAudioIndex === null && stream.IsDefault);
                                  return (
                                    <div 
                                      key={stream.Index} 
                                      className={`track-option ${isActive ? 'active' : ''}`}
                                      onClick={() => {
                                        if (videoRef.current) {
